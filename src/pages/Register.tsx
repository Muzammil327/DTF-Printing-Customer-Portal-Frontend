...
const Register = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      setIsLoading(false);
      if (requestAccount) {
        alert("Account request submitted! You'll receive an email once approved.");
        navigate("/login");
      } else {
        navigate("/dashboard");
      }
      ...
      <div className="w-full max-w-md">
        ...
        <div className="text-center mb-8">
          <Link to="/" className="inline-flex items-center space-x-2">
            ...
            <span className="text-2xl font-bold text-slate-800">DTF Print Hub</span>
          </Link>
        </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="flex items-center space-x-2">
                <Checkbox
                  ...
                  onCheckedChange={(checked) => setRequestAccount(checked === true)}
                />
                <Label htmlFor="requestAccount" className="text-sm text-slate-600">
